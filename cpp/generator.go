...
func (me *stringWriter) Write(p []byte) (int, error) {
	*me = stringWriter(p)
	return len(p), nil
}
...
func (me *Cpp) cyborgbearDefsBody(headername string) string {
	td := map[string]string{
		"Header": headername,
		"Namespace": me.namespace,
		"Version": common.CyborgbearVersion,
	}
	// TODO: pass errors along
	file := box.MustString("cyborgbear.cpp.tmpl")
	tmpl := template.Must(template.New("Cpp").Parse(file))
	var retval stringWriter
	tmpl.Execute(&retval, td)
	println(file)
	println(string(retval))
